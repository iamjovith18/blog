    ...
    <div class="l-container u-clear">
        ...
        <main class="l-main js-main">
            ...
            {{csrf_field()}}
                ...
                <div class="form-body">
                    <input type="text" name="title" value="{{Request::old('title')}}" id="title" class="input input-text">
                </div>
                <label for="contents" class="form-title">CONTENTS</label>
                <div class="form-textarea">
                    <textarea name="content" id="inquiry" cols="30" rows="10" class="input input-contents">{{Request::old('content')}}</textarea>
                </div>
                @if(count($errors)>0)
                    @foreach ($errors->all() as $error)
                        <div class="alert alert-danger text-center"><strong>{{$error}}</div>
                    @endforeach
                @endif
